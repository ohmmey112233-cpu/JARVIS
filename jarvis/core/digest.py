...
# ต่ำกว่านี้ = ไม่ต้องพูดถึงการเดินทางเลย
# โอมเดินจากหอไปโรงเรียน 2 นาที (เดินเองแล้ววัดจริง 20 ส.ค. 2026 — พิกัดห่างกัน 165 ม.)
# ตัวเลข 12/15 นาทีในตัวอย่างของ kit เขียนไว้ก่อนที่จะมีใครวัดจริง
# หลักการข้อ 6 บอกว่าตัวเลขต้องกลายเป็นการตัดสินใจ — "ออก 07:25 เพื่อเดิน 2 นาที"
# ไม่ใช่การตัดสินใจ มันคือบรรทัดที่กินที่หน้าจอทุกเช้าโดยไม่เพิ่มอะไร
# ตั้งเป็นเกณฑ์แทนที่จะลบโค้ดทิ้ง เพราะขา โรงเรียน→จอมทอง เย็นศุกร์ (48 กม.)
# ยังเป็นการเดินทางจริงที่รถติดมีผล และถ้าวันหน้าโอมย้ายหอ บรรทัดนี้จะกลับมาเอง
MIN_REPORTABLE_TRAVEL_MINUTES = 5
...
def _travel_lines(ctx: DigestContext) -> list[str]:
    ...
    if minutes is not None and minutes <= 0:
        ...
        minutes = None
    if minutes is not None and minutes < MIN_REPORTABLE_TRAVEL_MINUTES:
        # ใกล้เกินกว่าจะต้องวางแผน — เงียบทั้งหัวข้อ รวมถึงบรรทัด "ออก ..."
        # ที่คำนวณจากมัน (หลักการข้อ 4: ไม่มีอะไรให้ตัดสินใจก็ไม่ต้องพูด)
        return []
