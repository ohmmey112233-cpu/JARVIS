...
# งวดที่ 2 — ประกอบ digest จริง + fetchers + backup + cron (20 ส.ค. 2026)

**ผลรวม: 350 เทสต์ผ่านทั้งหมด**

## ✅ พิสูจน์แล้วด้วยการรันจริง

| เรื่อง | หลักฐาน |
|---|---|
| ประกอบ digest จาก DB จริง | `assemble.py` (12 เทสต์) — ตารางสัปดาห์ตรงแบบ kit ทุกแถว รวมแถวพุธ `⚠️ ตื่น 06:30 + บ่าย ร.ด.` ที่เคยหายเพราะบั๊ก |
| `cli digest` + กันส่งซ้ำ | ยิงซ้ำวันเดิม = stdout ว่างสนิท exit 0 → cron จะไม่ส่งข้อความซ้ำ / telegram ไม่นับโควตา line นับ (H14) |
| fetchers → assemble → render ทั้งเส้น | `720s → 12 นาที → "ออก 07:13 ถึงสบายๆ"` ไหลครบ / API ที่ล่มหายเฉพาะบรรทัดตัวเอง (E2) |
| backup → ลบทิ้ง → restore | รันจริงใน sandbox: lessons กลับมาครบ, `integrity_check: ok`, ของเดิมถูกย้ายเก็บไม่ถูกลบ |
| cron setup ทั้งชุด | รันกับ hermes ปลอม: guard timezone + doctor ทำงาน, helper 4 ตัวถูก generate แล้ว **รันจริง** ได้ digest ภาษาไทยถูกต้อง, รันซ้ำ = เงียบ |
| teardown เลือกลบเฉพาะของตัวเอง | ลบ `jarvis-p1-*` / ปล่อย `jarvis-nightly-backup` ไว้ |

## การตัดสินใจเชิงออกแบบที่ควรรู้ตอนรับช่วง

1. **digest ไม่ผ่าน LLM เลยทั้งเส้น** — cron ใช้ `--no-agent` ส่ง stdout เข้าแชทตรงๆ
   เหตุผล: kit กำหนดรูปแบบตายตัว ให้โมเดลเรียบเรียงใหม่ทุกเช้า = ดริฟต์ + เปลืองเงิน
2. **consolidation ความจำเกิดตอน 06:18 พร้อมรายงานความฝัน** (ไม่ใช่ตี 2 แยก job)
   — `digest dream` จัดระเบียบ+รายงานจบในคำสั่งเดียว ไม่มีไฟล์สถานะข้ามกัน
   ถ้าใครเพิ่ม job consolidate แยกตอนตี 2 **รายงานเช้าจะว่างตลอดกาล** (ของถูกเก็บ
   ไปก่อนแล้ว) — ห้ามเพิ่ม
3. **`--record` เป็นของ cron เท่านั้น** — รันตามคำขอโอมห้ามใส่ ไม่งั้นขอ digest
   ตอนตี 5 จะไปกินสิทธิ์ของรอบ 06:20 ทำให้รอบจริงเงียบ (กติกาอยู่ใน skill แล้ว)
4. ตาราง cron: เช้า `20 6 * * 0,1,2,4,5,6` + พุธแยก `0 6 * * 3` / ความฝัน 06:18 /
   เย็น 18:30 / สัปดาห์ อาทิตย์ 20:00 / backup 03:30 (`docs/backup.md`)

## ⚠️ ยังพิสูจน์ไม่ได้จากที่นี่

| เรื่อง | ตรวจเมื่อไหร่ |
|---|---|
| parser ของ fetchers กับ response จริง | ต่อ API จริงบน VPS — ทุก parser ติดป้าย ⚠️ ไว้แล้ว แก้ทีละฟังก์ชัน |
| `hermes cron create` รับ flag ตามที่ setup ใช้จริง | ตอนรัน setup บน VPS (ทดสอบกับ hermes ปลอมเท่านั้น) |
| อัปโหลด backup ออกนอก VPS ผ่าน rclone | ตอนตั้ง R2/Drive จริง |
| Google Calendar fetcher | Phase 2 — ต้องมี OAuth ก่อน ตอนนี้ dormant |
